...
router.post("/", async (req, res, next) => {
  try {
    ...
    const id = await createParticipant({ name, email, phone }, req.user.id);
...
router.get("/", async (req, res, next) => {
  try {
    ...
    const ownerFilter = req.user?.role === "student" ? req.user.id : undefined;
    const [participants, total] = await Promise.all([
      listParticipants({ skip, limit: actualLimit, ownerId: ownerFilter }),
      countParticipants(),
    ]);
